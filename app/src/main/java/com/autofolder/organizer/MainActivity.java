...
public class MainActivity extends Activity {
    ...
    void scan(){
        ...
        for(String c:CategoryEngine.ORDER){Integer n=counts.get(c); if(n!=null) {TextView t=tv("• "+c+" — "+n+" apps",16); t.setTextColor(Color.WHITE); list.addView(t);}}
    }
    void startOrganization(){
        if(apps==null) scan();
        getSharedPreferences(PREFS,0).edit().putBoolean("running",true).apply();
        Toast.makeText(this,"Organization started. Keep the phone unlocked and do not touch the screen while it runs.",Toast.LENGTH_LONG).show();

...
public class HomeOrganizerAccessibilityService extends AccessibilityService {
    ...
    private boolean startedForRun=false;
    ...
    @Override public void onAccessibilityEvent(AccessibilityEvent event){
        ...
        if(!isRunning() || busy || startedForRun) return;
        String pkg = event.getPackageName()==null ? "" : event.getPackageName().toString();
        // MainActivity intentionally sends the user back to the existing launcher.
        // The accessibility service may already be connected at that point, so
        // onServiceConnected() alone is not sufficient to start a newly requested run.
        if(!homePackage.isEmpty() && homePackage.equals(pkg)) {
            startedForRun=true;
            toast("AutoFolder is starting. Keep the phone unlocked and don't touch the screen.");
            h.postDelayed(this::runNextCategory, 900);
        }
    }
    ...
    private void finish(){ startedForRun=false; getSharedPreferences(PREFS,0).edit().putBoolean("running",false).apply(); toast("AutoFolder finished. Apps were only moved as home-screen shortcuts; nothing was uninstalled."); }
    private void stopRun(){ startedForRun=false; getSharedPreferences(PREFS,0).edit().putBoolean("running",false).apply(); }
    ...
    private void performLongDrag(float sx,float sy,float ex,float ey,Runnable done){
        // A launcher drag normally requires a real long-press before the finger moves.
        // The previous build moved immediately, which could simply scroll the app drawer.
        Path hold=new Path(); hold.moveTo(sx,sy); hold.lineTo(sx,sy);
        GestureDescription.StrokeDescription press = new GestureDescription.StrokeDescription(hold,0,650,true);
        Path move=new Path(); move.moveTo(sx,sy); move.lineTo(ex,ey);
        GestureDescription.StrokeDescription drag = press.continueStroke(move,0,950,true);
        dispatchGesture(new GestureDescription.Builder().addStroke(drag).build(),new GestureResultCallback(){@Override public void onCompleted(GestureDescription g){done.run();}@Override public void onCancelled(GestureDescription g){toast("Launcher rejected a drag; stopping safely.");stopRun();done.run();}},null);
    }
